...
export default function LahoreCheckout() {
  const navigate = useNavigate();
  const location = useLocation();
  const [isLoading, setLoading] = useState(false);
  const { bookingDetails } = location.state || {};

  const totalPrice = bookingDetails?.totalprice + 14;
  const selectedDate = bookingDetails?.selectedDate;

  const count = bookingDetails?.totalCount;
