...
export default function UmrahPayment({ ...props }) {
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState(null);
  const [timeLeft, setTimeLeft] = useState(299);
  const [isLoading, setIsLoading] = useState(true);
  const navigate = useNavigate();
  const location = useLocation();
  const { details, contactDetails } = location.state || {};
        ...
        <>
          ...
          <div className="w-full bg-[#F2F4F7] h-auto pb-11 px-2 sm:px-4 lg:px-16 pr-4 pt-[22px]">
            <div className="flex flex-col lg:flex-row lg:justify-between gap-4">
              ...
              <div className="w-full sm:w-[48%] md:w-[32%] lg:w-[31.6%] h-max rounded-[10px] bg-white border-[1px] border-[#D2D2D2] lg:ml-[14px] pb-4">
                <h2 className="text-[18px] font-semibold ml-4 mt-2">
                  20 Days Umrah Package (22 Dec to 11 Jan)
                </h2>

                <div className="w-full flex justify-between items-center px-4 pl-1 pt-3 border-t-[1px] mt-3 border-[#D0D5DD]">
                  <h2 className="text-[18px] font-semibold ml-3">Total</h2>
